...
export const testResult = async (req, res) => {
  try {
    const { department, year, stuId } = req.body;
    console.log(req.body);
    const errors = { notestError: String };
    const student = await Student.findOne({ department, year, stuId });
    console.log(student);
    const test = await Test.find({ department, year, stuId });
    console.log(test);
    // if (test.length === 0) {
